...
def setup_model(model_config):
    ...
    full_path_to_model_checkpoint = os.path.join(
        BASE_DIR, model_config['full_path_to_model_checkpoint'])
    ...
    if os.path.isfile(full_path_to_model_checkpoint):
        ...
        hf_hub_download(repo_id='asigalov61/Allegro-Music-Transformer',
                        ...
                        local_dir=os.path.join(BASE_DIR, '../Models/Tiny/'),
...
def model_stats(model_config, model):
    ...
    if model_config['plot_tokens_embeddings']:
        ...
        plt.plot()
        plt.savefig(
            os.path.join(BASE_DIR, "content/Allegro-Music-Transformer-Small-Tokens-Embeddings-Plot.png"), bbox_inches="tight")
...
def process_seed_midi(seed_midi_config, midi_file):
    ...
    if midi_file:
        score = TMIDIX.midi2single_track_ms_score(
            midi_file, recalculate_channels=False)
        f = midi_file
    ...
    if f != '':
        ...
        if len(events_matrix1) > 0:
            ...
            instruments_list_without_drums = list(
                set([y[3] for y in events_matrix1 if y[3] != 9]))
            ...
            if len(events_matrix1) > 0 and len(instruments_list_without_drums) > 0:
                ...
                mode_dur = statistics.mode(
                    [y[1] for y in melody_chords if y[2] != 9])
        ...
        detailed_stats = TMIDIX.Tegridy_ms_SONG_to_MIDI_Converter(song_f,
                                                                  ...
                                                                  output_file_name=os.path.join(
                                                                      BASE_DIR, 'content/Allegro-Music-Transformer-Seed-Composition'),
                                                                  track_name='Project Los Angeles',
                                                                  list_of_MIDI_patches=[
                                                                      0, 24, 32, 40, 42, 46, 56, 65, 73, 0, 53, 19, 0, 0, 0, 0]
                                                                  )
        ...
        colors = ['red', 'yellow', 'green', 'cyan', 'blue', 'pink',
                  'orange', 'purple', 'gray', 'white', 'gold', 'silver']
        ...
        if seed_midi_config['render_MIDI_to_audio']:
            FluidSynth(os.path.join(BASE_DIR, "FluidR3_GM.sf2"), 16000).midi_to_audio(
                str(fname + '.mid'), str(fname + '.wav'))
...
def perform_inpainting(inpainting_config, model, melody_chords_f, ctx):
            ...
            if ((melody_chords_f[i]-1280) // 128) in inpaint_instrument:
                ...
                for j in range(number_of_samples_per_inpainted_note):
                    ...
                    inp = torch.LongTensor(
                        [out2[-number_of_memory_tokens:]]).cuda()
    ...
    if len(out2) != 0:
        ...
        detailed_stats = TMIDIX.Tegridy_ms_SONG_to_MIDI_Converter(song_f,
                                                                  output_signature='Allegro Music Transformer',
                                                                  output_file_name=os.path.join(
                                                                      BASE_DIR, 'content/Allegro-Music-Transformer-Composition'),
                                                                  track_name='Project Los Angeles',
                                                                  list_of_MIDI_patches=[
                                                                      0, 24, 32, 40, 42, 46, 56, 65, 73, 0, 53, 19, 0, 0, 0, 0]
                                                                  )
...
if __name__ == "__main__":
    ...
    config = load_config(os.path.join(BASE_DIR, 'parameters.json'))
    ...
    f = os.path.join(BASE_DIR, "piano.midi")
    ...
    if any(value for key, value in config['inpainting'].items() if key != "number_of_memory_tokens" and key != "number_of_samples_per_inpainted_note"):
        inpainted_music = perform_inpainting(
            config['inpainting'], model, melody_chords, ctx)
    else:
